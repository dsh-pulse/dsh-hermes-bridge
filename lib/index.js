  ...
  if (sessionId) {
    ...
    try {
      handle = await ctx.agents.resume({
        resumeSessionId: sid,
        agentOptions: { provider: runtime.provider, ...(runtime.model ? { model: runtime.model } : {}) },
...
async function executeTask(ctx, taskText, context, cwd, sessionId, title) {
  ...
  return withLock(lockKey, async () => {
    ...
    // turn 错误检测：agent turn 以 error 结束时 whenIdle 也会返回，必须显式检查
    // （否则会误报 done，如 {{model}} 模板变量未解析等）
    for (let i = events.length - 1; i >= 0; i--) {
      const ev = events[i]
      if (ev?.type === 'turn/end') {
        const reason = ev?.data?.reason
        if (reason?.kind === 'error') {
          throw new Error(`agent 执行出错: ${reason.error?.message || JSON.stringify(reason).slice(0, 300)}`)
        }
        break
      }
    }
